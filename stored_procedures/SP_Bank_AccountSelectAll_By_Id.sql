IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Bank_AccountSelectAll_By_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Bank_AccountSelectAll_By_Id]    
 @Account_Id bigint          
AS            
BEGIN            
SELECT     
...
WHERE BA.Status=0   and Account_Id=@Account_Id         
END
    ')
END;
