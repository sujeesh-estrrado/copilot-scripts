IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GET_DUE_NEW_FOR_TESTING]') 
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE procedure [dbo].[SP_GET_DUE_NEW_FOR_TESTING] --11829,804  --11829,275  
...
Create table #FeeCategoryTable (DueDate varchar(50),Balence decimal(18,2),Amount decimal(18,2))--- Temporary Table All Fee Categorys Creation  
...
begin        
insert into #FeeCategoryTable (DueDate,Balence,Amount) select --'''' as ItemDescription,'''' as FeeHeadId,        
...
begin        
insert into #FeeCategoryTable (DueDate,Balence,Amount) select convert(varchar(50),cdp.Duration_Period_To,103) as duedate,        
(select Balance from dbo.Tbl_Fee_Entry_Main where Candidate_Id=@CandidateId and   
IntakeId=@IntakeId and FeeHeadId=FSD.Feehead_Id and ItemDescription=FSD.ItemDescription)as balance,      
FSD.Amount       
...
begin        
insert into #FeeCategoryTable(DueDate,Balence,Amount) select distinct --FEM.ItemDescription,FEM.FeeHeadId,          
