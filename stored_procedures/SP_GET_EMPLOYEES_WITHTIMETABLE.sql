IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GET_EMPLOYEES_WITHTIMETABLE]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE PROCEDURE [dbo].[SP_GET_EMPLOYEES_WITHTIMETABLE]      
...
WHERE CT.Class_TimeTable_Status=0 AND E.Employee_Status=0     
      and CT.Del_Status=0
END
	');
END;
