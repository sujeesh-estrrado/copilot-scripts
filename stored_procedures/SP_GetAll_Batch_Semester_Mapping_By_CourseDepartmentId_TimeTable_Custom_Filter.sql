IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Batch_Semester_Mapping_By_CourseDepartmentId_TimeTable]')
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[SP_GetAll_Batch_Semester_Mapping_By_CourseDepartmentId_TimeTable]                  
@Course_Department_Id bigint                       
AS                        
BEGIN 

SELECT      
    CAST(cd.Duration_Period_Id AS VARCHAR(10)) + ''_'' + CAST(cd.Batch_Id AS VARCHAR(10)) AS UniqueId,
    cd.Duration_Period_Id,
    cd.Duration_Period_Id AS Duration_Mapping_Id, 
    bd.Batch_From, 
    bd.Batch_To, 
    CONCAT(bd.Batch_Code, ''-'', cs.Semester_Code) AS BatchSemester, 
    D.Department_Name, 
    D.Department_Id      
...
LEFT OUTER JOIN dbo.Tbl_Course_Category AS CC ON CC.Course_Category_Id = Cdep.Course_Category_Id 
