IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Class_Division_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[SP_GetAll_Class_Division_Id]    @Course_Category_Id bigint  
...
FROM         dbo.Tbl_Department INNER JOIN      
      ...
      INNER JOIN Tbl_Course_Duration_Mapping ON dbo.Tbl_Course_Department.Course_Department_Id = Tbl_Course_Duration_Mapping.Course_Department_Id
       ...
       where Tbl_Course_Department.Course_Department_Status=0 and      
                 ...
                 Tbl_Course_Department.Course_Category_Id = @Course_Category_Id)tbl                      
where tbl.num=1           
