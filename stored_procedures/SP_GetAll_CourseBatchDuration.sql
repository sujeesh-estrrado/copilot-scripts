IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_CourseBatchDuration]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_GetAll_CourseBatchDuration] 
(@facultyid bigint=0,
...
BEGIN                            
  ...
  begin                          
		...
		FROM dbo.Tbl_Course_Batch_Duration CBD              
			...
			LEFT JOIN dbo.fee_group FG ON FG.ProgramIntakeID = CBD.Batch_Id -- Join with fee_group
