IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Course_Department_Faculty_Custom_Filter]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE PROCEDURE [dbo].[SP_GetAll_Course_Department_Faculty_Custom_Filter]                                
    @Faculty_Id NVARCHAR(MAX) -- Comma-separated list of Faculty IDs                       
AS                                 
BEGIN                         
    DECLARE @FacultyIdTable TABLE (Faculty_Id BIGINT);
    INSERT INTO @FacultyIdTable (Faculty_Id)
    SELECT value FROM dbo.SplitStringFunction(@Faculty_Id, '','');
    ...
    WHERE 
        ...
        [Tbl_Course_Level].Course_Level_Id IN (SELECT Faculty_Id FROM @FacultyIdTable)
    ORDER BY 
        Department_Name                              
END');
END;