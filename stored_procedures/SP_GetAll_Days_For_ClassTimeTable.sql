IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Days_For_ClassTimeTable]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE PROCEDURE [dbo].[SP_GetAll_Days_For_ClassTimeTable]   
@Dept_Id     BIGINT,  
@BatchID     BIGINT,
...
BEGIN   
  ...
  SELECT DISTINCT 
		   ...
		   INNER JOIN Tbl_Class_TimeTable    CT  ON  CCTM.Customize_ClassTimingId=CT.Class_Timings_Id AND CCTM.Days_Id=CT.Day_Id
