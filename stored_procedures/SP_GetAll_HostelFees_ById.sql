IF NOT EXISTS (
    SELECT 1 
    FROM sys.objects 
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_HostelFees_ById]') 
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE PROCEDURE [dbo].[SP_GetAll_HostelFees_ById]  
        ...
        FROM (          
            ...
            INNER JOIN Tbl_Hostel_Fee HF ON HF.HostelFeeId = fd.HostelFeeId  
            INNER JOIN Tbl_HostelRegistration HR ON HR.Hostel_Id = HF.HostelId  
            WHERE f.Delete_Status = 0 AND f.Hostel_Fee_Payment_Id = @Hostel_Fee_Payment_Id
        ) t        
        WHERE t.RNO = 1                              
        END
    ')
