IF NOT EXISTS (
    SELECT 1 
    FROM sys.objects 
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Placement_Details]') 
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE PROCEDURE [dbo].[SP_GetAll_Placement_Details]     
        AS    
        BEGIN     
            SELECT 
                PD.Placement_Details_Id,   -- Explicit column names for PD
                PD.Company_Id,             
                PD.Placement_Date,         
