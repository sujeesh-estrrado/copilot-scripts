IF NOT EXISTS (
    SELECT 1 
    FROM sys.objects 
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_GetAll_Salary_Verified]') 
    AND type = N'P'
)
BEGIN
    EXEC('
     CREATE procedure [dbo].[SP_GetAll_Salary_Verified]  
...
BEGIN  
 ...
 SELECT     dbo.Tbl_Employee.Employee_FName +'' ''+ dbo.Tbl_Employee.Employee_LName AS Employee, dbo.Tbl_Salary_Verified.Sal_Verified_id,   
                      dbo.Tbl_Salary_Verified.Sal_Employee_Id, dbo.Tbl_Salary_Verified.Sal_Month_Year, dbo.Tbl_Salary_Verified.Sal_Verified_Date  
