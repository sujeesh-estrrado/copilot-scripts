IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_CandidateContactDet]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[SP_Get_CandidateContactDet] 
(@Candidate_Id bigint)    
as    
begin    
...
--CONCAT(CC.Candidate_PermAddress,''  '',CC.Candidate_PermAddress_Line2,''  '',CC.Candidate_PermAddress_postCode
--,''  '',PS.Country,''  '',PCS.State_Name,''  '',PCC.City_Name,PCC1.City_Name) as Permenent_Address

CONCAT(CC.Candidate_PermAddress,''  '',CC.Candidate_PermAddress_Line2,''  '',CC.Candidate_PermAddress_postCode
,''  '',PCS.State_Name,''  '',PCC.City_Name,PCC1.City_Name) as Permenent_Address


,CONCAT(employee_fname,'' '',employee_Lname)as counsellorName,
...
Candidate_Email,'''' as Candidate_Telephone,'''' as Candidate_Mob1,'''' as Candidate_Mob2,
(case when CPD.Disability_Chkbox_Status=''1''then CPD.Disability_Type when CPD.Disability_Chkbox_Status=''0'' then ''-NA-'' end ) as Disability,
'''' as Mailing_Address1,'''' as CountryCode,'''' as StateCode,'''' as Candidate_ContAddress_City,'''' as CityName,CCS.State_Name [State], CS.Country
    from  dbo.Tbl_Candidate_ContactDetails CC
        left join dbo.Tbl_Candidate_Personal_Det CPD  on CPD.Candidate_Id=CC.Candidate_Id 
        ...
        left join dbo.Tbl_City PCC1 on PCC1.City_Id = CPD.candidate_city
