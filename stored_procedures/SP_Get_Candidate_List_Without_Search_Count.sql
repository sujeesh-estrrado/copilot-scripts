IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Candidate_List_Without_Search_Count]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE PROCEDURE [dbo].[SP_Get_Candidate_List_Without_Search_Count]                                  
    AS                                  
    BEGIN     
        SELECT 
