IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Count_Agent_Enquiry_List_new]') 
    AND type = N'P'
)
BEGIN
    EXEC('
   CREATE procedure [dbo].[SP_Get_Count_Agent_Enquiry_List_new] --'' '',''To_RAISE_INVOICE'',2
  (                         
...
where (ApplicationStatus=''Preactivated'' or ApplicationStatus=''Completed'') and--CPD.Candidate_DelStatus=0 and 
CPD.Agent_ID=@Agent_ID1 and   (Invoice_Status IS NULL   or     Invoice_Status=''''))Base )A                           
       begin        
        SELECT                       
            count(ID)counts                                                 
        FROM                                     
...
LEFT JOIN Tbl_Employee E on E.Employee_Id= (Select Employee_Id from dbo.Tbl_Employee_User where [User_Id] = SR.UserId)                         
   left join dbo.Tbl_Student_Health_General SG ON  SG.StudentId = CPD.Candidate_Id                 
left join  Tbl_Employee EC on (EC.Employee_Id=CPD.CounselorEmployee_id and EC.Employee_Status=0)
where --CPD.Candidate_DelStatus=0 and
 CPD.Agent_ID=@Agent_ID1 and   (Invoice_Status IS NULL   or     Invoice_Status=''''))Base )A                           
       begin        
        SELECT                       
            count(ID)counts                                                 
