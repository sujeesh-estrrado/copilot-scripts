IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Course_Seat_Capacity_BY_Course_Department_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Get_Course_Seat_Capacity_BY_Course_Department_Id](@Course_Department_Id bigint)
...
and a.Seat_Capacity_Status=0 and dbo.Tbl_Course_Department.Course_Department_Id=@Course_Department_Id;
END
    ');
END;
