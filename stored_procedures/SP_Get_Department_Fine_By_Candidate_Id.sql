IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Department_Fine_By_Candidate_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Get_Department_Fine_By_Candidate_Id]    
@Candidate_Id bigint   
AS    
...
Where DF.Candidate_Id=@Candidate_Id and Department_Fine_Payment_Id is null     
END 
	');
END;
