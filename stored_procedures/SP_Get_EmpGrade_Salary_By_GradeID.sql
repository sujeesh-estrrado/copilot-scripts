IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_EmpGrade_Salary_By_GradeID]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Get_EmpGrade_Salary_By_GradeID]        
(@Grade_Id bigint, @Emp_Id bigint)            
...
WHERE (sh.Salary_head_Delete_Status = 0) AND (Grade_Id=@Grade_Id) AND (Emp_Id=@Emp_Id)  
END
');
END;
