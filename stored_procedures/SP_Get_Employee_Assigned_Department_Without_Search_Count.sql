IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Employee_Assigned_Department_Without_Search_Count]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Get_Employee_Assigned_Department_Without_Search_Count]                                    
AS                                    
BEGIN       
                                   
SELECT ROW_NUMBER() OVER (ORDER BY Emp_DepartmentAllocation_Id DESC) AS RowNumber,    
   ECA.Employee_Id as ID,    
   E.Employee_FName+'' ''+E.Employee_LName as [Employee Name],      
   ED.Dept_Name as [Employee Department],          
   CC.Course_Category_Name+''-''+D.Department_Name as [Assigned Departments]  
