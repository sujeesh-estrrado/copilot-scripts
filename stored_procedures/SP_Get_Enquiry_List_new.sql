IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Enquiry_List_new]') 
    AND type = N'P'
)
BEGIN
    EXEC('
  CREATE procedure [dbo].[SP_Get_Enquiry_List_new]-- 1,100,''de'',''--select--'',''--select--'',''--select--'','''','''',''active''              
(                  
...
and(CPD.Active_Status=@ActiveStatus)       
)Base )A      
 SELECT                         
ID,                           
            BloodGroup,                                                   
