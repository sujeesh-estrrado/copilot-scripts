IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Enquiry_List_newtest]')
    AND type = N'P'
)
BEGIN
    EXEC('
   CREATE procedure [dbo].[SP_Get_Enquiry_List_newtest]  
(      
@CurrentPage int=null ,                     
@PageSize int=null   ,                  
@SearchTerm varchar(100),  
@CounselorCampus varchar(50) ,  
...
left join  Tbl_Employee EC on (EC.Employee_Id=CPD.CounselorEmployee_id and EC.Employee_Status=0)where CPD.Candidate_DelStatus=0)Base )A    
...
or EmailID like  ''''+ @SearchTerm +''%''  
   or @SearchTerm='''')           
  and (CounselorCampus=@CounselorCampus or  @CounselorCampus=''--Select--'')  
and (TypeOfStudent=@Type1 or @Type1=''--Select--'')  
and (Employee_Name=@EmployeeName or @EmployeeName=''--Select--'')  
          and RowNumber >  CONVERT(VARCHAR,@LowerBand)   AND RowNumber <  CONVERT(VARCHAR, @UpperBand)           
       end     
