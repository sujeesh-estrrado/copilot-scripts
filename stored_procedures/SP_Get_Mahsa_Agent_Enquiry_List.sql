IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Mahsa_Agent_Enquiry_List]')
    AND type = N'P'
)
BEGIN
    EXEC('
  CREATE procedure [dbo].[SP_Get_Mahsa_Agent_Enquiry_List]-- 1,10,'''',''Active'',1563,1,0,0            
(                  
@CurrentPage int=1 ,                                 
@PageSize int=10 ,                              
...
SET NOCOUNT ON                                      
    ...
    --declare @commission bigint=0;
    --declare @pay bigint=0;                                           
...
SELECT * InTO #TEMP1 FROM                                                    
     ...
     Base.*  FROM                                                   
(SELECT CPD.Candidate_Id as ID,SG.BloodGroup,ApplicationStatus,                  
...
--CAST(COALESCE((select Sum(totalamountpaid) from student_bill b inner join Tbl_Commission_Settings S on S.Feehead_Id=b.accountcodeid
--where  studentid = CPD.Candidate_Id and Agent_ID=@Agent_ID1 and IntakeId=NA.Batch_Id and billcancel=0 and S.Delete_Status=0),0)AS DECIMAL(18,2)) as Amount1,

--CAST(COALESCE((select Sum(floatamount) from student_payment_float b inner join Tbl_Commission_Settings S on S.Feehead_Id=b.accountcodeid
--where  studentid = CPD.Candidate_Id and Agent_ID=@Agent_ID1 and IntakeId=NA.Batch_Id  and S.Delete_Status=0),0)AS DECIMAL(18,2)) as Amount2,
...
NA.Batch_Id,      
 ...
 SELECT distinct ID,  --RowNumber,                    
           
                                      
            BloodGroup, ApplicationStatus,                                                  
            CandidateName,        
         Employee_Name,                   
    ...
    [LevelName],                 
                [Category],                  
                 DepartmentID,                  
                Department,Commission_Amount,(Amount1+Amount2) as Amount_Paids,Amountpaids as Amount_Paid,Minimum_Amount,case when((Amount1+Amount2)>Minimum_Amount and Minimum_Amount>0) then 0 else 1 end as Payment_Clearance                                
 ...
 OFFSET @pagesize * (@CurrentPage - 1) ROWS
  ...
  --          where                
  --RowNumber >  CONVERT(VARCHAR,@LowerBand)   AND RowNumber <  CONVERT(VARCHAR, @UpperBand)    order by RowNumber asc   
