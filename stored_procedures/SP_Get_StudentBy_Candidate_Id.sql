IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_StudentBy_Candidate_Id]')
    AND type = N'P'
)
BEGIN
    EXEC('
create procedure [dbo].[SP_Get_StudentBy_Candidate_Id]   
(@Candidate_Id bigint)  
as  
begin   
...
where CP.Candidate_Id=@Candidate_Id and SR.Student_Reg_Status=0 and CP.Candidate_DelStatus=0  
end
    ')
END;
