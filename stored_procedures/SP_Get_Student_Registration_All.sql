IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Student_Registration_All]') 
    AND type = N'P'
)
BEGIN
    EXEC('
           
CREATE procedure [dbo].[SP_Get_Student_Registration_All]                   
(                  
...
BEGIN            
 ...
 SET @SqlString=''WITH tempProfile AS                  
        (SELECT ROW_NUMBER() OVER (ORDER BY Candidate_Id) AS RowNumber ,t.*       
     
    
FROM  (SELECT DISTINCT sr.Student_Reg_No as [RegNo],                                   
       sr.Student_Reg_Id,  
    sr.Candidate_Id,                             
