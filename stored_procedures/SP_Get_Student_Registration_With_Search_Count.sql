IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Student_Registration_With_Search_Count]')
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[SP_Get_Student_Registration_With_Search_Count]              
(              
...
SELECT ROW_NUMBER() OVER (ORDER BY Student_Reg_No) AS RowNumber,  
t.[Reg No],  
t.[Name],  
t.Gender,  
t.[Class-Division],  
...
FROM  (SELECT DISTINCT sr.Student_Reg_No,    
...
or    [Class-Division] like  ''''+ @SearchTerm+''%''            
