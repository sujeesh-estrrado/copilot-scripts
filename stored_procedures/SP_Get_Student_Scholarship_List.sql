IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Student_Scholarship_List]')
    AND type = N'P'
)
BEGIN
    EXEC('
 create procedure [dbo].[SP_Get_Student_Scholarship_List]                
AS              
BEGIN              
SELECT ss.*,CD.Course_Category_Id,CC.Course_Category_Name+''-''+d.Department_Name as [Class-Division],
