IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Get_Students_List_With_Search_Count]')
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[SP_Get_Students_List_With_Search_Count]    
(    
...
and  ([Student Name] like  ''''+ @SearchTerm+''%''    
    
or  [Identification Number]  like  ''''+ @SearchTerm+''%''     
    
or  [Blood Group] like  ''''+ @SearchTerm+''%''    
    
or  [Admission Number] like  ''''+ @SearchTerm+''%''     
