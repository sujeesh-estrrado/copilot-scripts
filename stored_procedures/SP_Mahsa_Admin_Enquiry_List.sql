IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Mahsa_Admin_Enquiry_List]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Mahsa_Admin_Enquiry_List] --1,10,'''',''Active'',1635,0,0,0              
( 
...
D.Department_Id=NA.Department_Id)  end  as Department,CPD.Agent_ID,  
 ...
 where  studentid = CPD.Candidate_Id and Agent_ID != 0 and IntakeId=NA.Batch_Id and S.Delete_Status=0),0)AS DECIMAL(18,2)) as Amount1,  
 0.00 as Amount2, CAST(COALESCE((select Sum(amount)-Sum(adjustmentamount)  from student_payment b   
 where  studentid = CPD.Candidate_Id and CPD.Agent_ID != 0),0)AS DECIMAL(18,2)) as Amount_Paid , 
  
 CAST(COALESCE((select Sum(amount)-Sum(adjustmentamount)  from student_payment b   
 where  studentid = CPD.Candidate_Id and CPD.Agent_ID != 0),0)AS DECIMAL(18,2)) as Amountpaids,  
...
(select cbd.Batch_Code from Tbl_Course_Batch_Duration cbd where                        
cbd.Batch_Id=NA.Batch_Id)  end  as Batch_Code,                        
...
where  (CPD.Campus= @organization_id or @organization_id=''0'')  
and (NA.Department_Id= @Department_id  or @Department_id=''0'')  
--and (cbd.IntakeMasterID= @intake_id or @intake_id=''0'')
AND (CPD.Candidate_Fname+'' ''+CPD.Candidate_Lname like ''''+ @SearchTerm +''%''         
or D.Department_Name like ''''+@SearchTerm+''%'' or CPD.AdharNumber like ''''+ @SearchTerm+''%''        
or CCat.Course_Category_Name like  ''''+ @SearchTerm +''%'' or CC.Candidate_Mob1 like  ''''+ @SearchTerm +''%'' or CC.Candidate_Email like  ''''+ @SearchTerm +''%'' or @SearchTerm='''')                
