IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Marketing_StudentListing_Report_Lead]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE PROCEDURE [dbo].[SP_Marketing_StudentListing_Report_Lead] -- ''Pending'',0,0,0,'''','''','''',null,null,1,100000    
(    
    @Status VARCHAR(MAX) = '''',    
    @Staff BIGINT = 0,    
...
BEGIN    
    ...
    BEGIN    
        ...
        WHERE (@Staff = 0 OR CounselorEmployee_id = @Staff)    
          ...
          AND ((CONVERT(DATE, RegDate) >= @fromdate AND CONVERT(DATE, RegDate) < DATEADD(DAY, 1, @todate))     
