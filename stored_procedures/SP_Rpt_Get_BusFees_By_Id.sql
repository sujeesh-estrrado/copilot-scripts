IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Rpt_Get_BusFees_By_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Rpt_Get_BusFees_By_Id]                        
...
Select * from (      
Select    
ROW_NUMBER() OVER  (PARTITION BY f.Transport_Fees_Id Order by f.Transport_Fees_Id) AS RNO,                      
f.Transport_Fees_Id,                      
...
STUFF((SELECT distinct '', '' + CONVERT(CHAR(4), t1.[month], 100) + CONVERT(CHAR(4), t1.[month], 120)       
         from Tbl_Transport_Fees_Details t1      
         where f.Transport_Fees_Id = t1.Transport_Fees_Id      
            FOR XML PATH(''''), TYPE      
            ).value(''.'', ''NVARCHAR(MAX)'')       
        ,1,2,'''') [Month]      
...
WHERE  f.Delete_Status=0 and f.Transport_Fees_Id=@Transport_Fees_Id) t    
Where t.RNO=1                          
END
');
END;