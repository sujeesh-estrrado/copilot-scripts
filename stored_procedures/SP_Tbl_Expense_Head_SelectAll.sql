IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[SP_Tbl_Expense_Head_SelectAll]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[SP_Tbl_Expense_Head_SelectAll]              
AS              
BEGIN              
SELECT             
(Select dbo.GetInvoiceCodeById_Code(Invoice_Code_Id,Inventory_Invoice_code)) As Inventory_Invoice_code,          
   Expense_Id,Acc_Cat_Id,Expense_Title,Expense_Description,Expense_Amount,Expense_Date ,        
(Select  Acc_Cat_Name from  Tbl_Income_Expense_Category A  where  A.Acc_Cat_Id =EH.Acc_Cat_Id) as Head,            
Case                
...
Else '''' End AS Payment_Mode                       
  ...
  FROM  Tbl_Expense_Head EH            
INNER JOIN Tbl_Payment_Details  p ON p.Payment_Details_Particulars_Id =EH.Expense_Id  and  p.Payment_Details_Particulars=''EXPENSE''             
...
Order by Expense_Date desc,Expense_Id desc    END
    ')
END;
