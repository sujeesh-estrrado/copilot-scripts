IF NOT EXISTS (
    SELECT 1 
    FROM sys.objects 
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_GetAll_Library_Fine_By_Candidate_Id]') 
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE PROCEDURE [dbo].[Sp_GetAll_Library_Fine_By_Candidate_Id] 
        @Candidate_Id BIGINT        
        AS          
        BEGIN          
            SELECT 
                ...
                B.Book_Serial_No,          
                CASE 
                    WHEN Candidate_or_Employee = 0 THEN 
            ...
            WHERE 
                ...
                AND Candidate_or_Employee = 0 
                AND Candidate_Employee_Id = @Candidate_Id  
        END
    ')
END
