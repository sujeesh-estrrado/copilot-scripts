IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_GetById_Invoice_studentid]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[Sp_GetById_Invoice_studentid] --1,''INV409''
    -- [dbo].[Sp_GetById_Invoice_studentid] 10,10
    @id bigint
    --,@DocumentNo varchar
AS
BEGIN
    SELECT DISTINCT 
                         TOP (100) PERCENT CPD.Candidate_Id AS ID,*,  CONCAT(CPD.Candidate_Fname ,'' '',CPD.Candidate_Lname)AS CandidateName,
                      ...
                      CPD.ApplicationStatus,   CASE WHEN NA.Batch_Id = 0 THEN ''Unspecified'' ELSE
                         ...
                         CASE WHEN NA.Department_Id = 0 THEN ''Unspecified'' ELSE
                             (SELECT        D .Department_Name
                               ...
                               ,O.Organization_Name,O.Organization_Address,O.Organization_City,O.Organization_Pin,cpd.IDMatrixNo,
                               O.Organization_Phone,O.Organization_Fax,O.Organization_GST_No,CONCAT(Employee_FName,'''',Employee_LName)as Employeename
