IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_All_Candidate_New_Visa]')
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE PROCEDURE [dbo].[Sp_Get_All_Candidate_New_Visa] 
        ...
        BEGIN
            ...
            BEGIN
                ...
                LEFT JOIN dbo.Tbl_Student_Semester AS SS ON SS.Candidate_Id = CPD.Candidate_Id AND SS.Student_Semester_Delete_Status = 0 AND SS.Student_Semester_Current_Status = 1
                ...
                LEFT JOIN dbo.Tbl_Employee AS E ON E.Employee_Id = (
                    SELECT Employee_Id FROM dbo.Tbl_Employee_User WHERE (User_Id = SR.UserId)
                )
                ...
                WHERE CPD.Candidate_DelStatus = 0 
                    ...
                    AND CPD.candidate_id NOT IN (SELECT candidate_id FROM tbl_visa_details WHERE delete_status = 0)
                ORDER BY ID DESC
