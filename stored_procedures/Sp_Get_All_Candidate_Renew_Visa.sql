IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_All_Candidate_Renew_Visa]')
    AND type = N'P'
)
BEGIN
    EXEC('
        CREATE PROCEDURE [dbo].[Sp_Get_All_Candidate_Renew_Visa] 
--''verified'',1,10,0,0,0,1,''
...
BEGIN
    ...
    BEGIN
        SELECT DISTINCT 
            ...
            CASE 
                WHEN dbo.tbl_approval_log.offer_letter_accept_date IS NOT NULL THEN CONVERT(VARCHAR(8), dbo.tbl_approval_log.offer_letter_accept_date, 3)
                ELSE ''-NA-'' 
            END AS offerletter_acceptdate,
            CASE 
                WHEN cn.PassportNo != '''' THEN ''Yes'' 
                ELSE ''No'' 
            END AS passportsts,
            tn.nationality
        ...
        LEFT OUTER JOIN dbo.Tbl_Student_Semester AS SS ON SS.Candidate_Id = CPD.Candidate_Id AND SS.Student_Semester_Delete_Status = 0 AND SS.Student_Semester_Current_Status = 1
        ...
        WHERE 
            ...
            AND (
                (CONCAT(CPD.Candidate_Fname, '' '', CPD.Candidate_Lname) LIKE ''%'' + @SearchTerm + ''%'')  
            ...
            AND (
                CPD.ApplicationStatus IN (''verified'', ''Verified'', ''Conditional_Admission'', ''Preactivated'', ''Completed'')
            )
            AND CPD.TypeOfStudent = ''INTERNATIONAL'' 
            AND CPD.candidate_id IN (SELECT candidate_id FROM tbl_visa_details WHERE delete_status = 0)
        ORDER BY ID DESC  
