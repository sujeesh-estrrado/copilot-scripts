IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_EmpStud_Library_Fine_Collected_By_EmpID]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[Sp_Get_EmpStud_Library_Fine_Collected_By_EmpID]    
...
SELECT ROW_NUMBER() OVER (ORDER BY [Issue_Book_Id] DESC) AS RowNumber,Base.*  FROM
    (SELECT DISTINCT  
...
LEFT JOIN Tbl_Employee E ON E.Employee_Id=BI.Candidate_Employee_Id  ) Base  
  
  
WHERE Base.Return_Date between @FromDate and @ToDate 

and  Candidate_or_Employee=@Candidate_or_Employee 

and  Candidate_Employee_Id=@Candidate_Employee_Id 
