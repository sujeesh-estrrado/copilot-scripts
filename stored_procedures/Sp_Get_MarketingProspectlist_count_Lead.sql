IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_MarketingProspectlist_count_Lead]')
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE PROCEDURE [dbo].[Sp_Get_MarketingProspectlist_count_Lead] --''Pending'',0,0,0,'''' ,'''' ,'''' ,null,null    
    (    
    @Status Varchar(MAX) = '''',
    @Staff BIGINT = 0,
    ...
    BEGIN
        ...
        BEGIN
            ...
            (
                ...
                FROM (
                    SELECT DISTINCT D.Candidate_Id, ApplicationStatus, ApplicationStage,
                    (CONCAT(UPPER(Candidate_Fname), '' '', UPPER(Candidate_Lname))) AS Candidatename, AdharNumber, Candidate_Gender,
                    Candidate_Email, Candidate_Mob1, Candidate_Telephone, Candidate_Guardian_Mob, Candidate_PermAddress,
                    Na.Nationality AS Candidate_Nationality, Department_Name, P.Course_Code, Diocese,
                    E.Employee_FName + '' '' + Employee_LName AS CounselorName, EnrollBy, Sponsorship,
                    A.Agent_Name, CONVERT(VARCHAR, RegDate, 103) AS RegDate
                    FROM Tbl_Lead_Personal_Det D
                    INNER JOIN Tbl_Lead_ContactDetails C ON C.Candidate_Id = D.Candidate_Id
            ...
            SELECT COUNT(Candidate_Id) AS totcount FROM #TEMP1
        END
        ...
        BEGIN
            ...
            SELECT COUNT(Candidate_Id) AS totcount FROM #TEMP3
        END
    END
    ')
END
