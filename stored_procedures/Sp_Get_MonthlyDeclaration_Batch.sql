IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_MonthlyDeclaration_Batch]')
    AND type = N'P'
)
BEGIN
    EXEC('
   CREATE procedure [dbo].[Sp_Get_MonthlyDeclaration_Batch]    
AS    
BEGIN    
    select cm.*,cbd.Batch_Code+''''+cs.Semester_Code as BatchName from dbo.LMS_Tbl_MonthDecl_BatchMapping cm  
