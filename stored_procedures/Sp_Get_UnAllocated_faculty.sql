IF NOT EXISTS (
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Get_UnAllocated_faculty]')
    AND type = N'P'
)
BEGIN
    EXEC('
   
CREATE procedure [dbo].[Sp_Get_UnAllocated_faculty]--  1,''2020-04-23 13:00:00'',''2020-04-23 14:00:00''                          
(             
@flag bigint=0,          
@from datetime=null,              
@to datetime=NULL,              
...
--and    (R.role_Id=3 or  R.role_Id=11)                      
 and E.Employee_Id  not in(                              
        select employee_Id from Tbl_Employee_Allocations where ( (Allocation_from>=@from and Allocation_To<=@to) or                             
...
if(@flag=1)          
begin          
select E.Employee_Id,CONCAT(E.Employee_FName,'' '',E.Employee_LName) as EmployeeName from Tbl_Employee E                              
inner join Tbl_Employee_User EU                              
on EU.Employee_Id=E.Employee_Id                              
inner join Tbl_User U on  U.user_Id=EU.User_Id                              
inner join tbl_Role R on R.role_Id=U.role_Id and E.Employee_Id not in(                              
...
on EU.Employee_Id=E.Employee_Id                              
inner join Tbl_User U on  U.user_Id=EU.User_Id                              
inner join tbl_Role R on R.role_Id=U.role_Id and E.Employee_Id  in(                              
select employee_Id from Tbl_Employee_Allocations where ( (Allocation_from>=@from and Allocation_To<=@to)                             
or                             
  (Allocation_from>=@from and Allocation_To<=@from) or                            
   (Allocation_from>=@to and Allocation_To<=@to) )   and Status=0)                            
