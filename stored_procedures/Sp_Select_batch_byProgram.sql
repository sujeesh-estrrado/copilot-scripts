IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_Select_batch_byProgram]') 
    AND type = N'P'
)
BEGIN
    EXEC('
CREATE procedure [dbo].[Sp_Select_batch_byProgram](@Pgmid bigint)
as
begin
SELECT        CDM.Duration_Mapping_Id, bd.Batch_From, bd.Batch_To, bd.Batch_Code + ''-'' + cs.Semester_Code AS BatchSemester, D.Department_Id
FROM            dbo.Tbl_Course_Duration_PeriodDetails AS cd LEFT OUTER JOIN
                         ...
                         dbo.Tbl_Department AS D ON D.Department_Id = Cdep.Department_Id where d.Department_Id=@Pgmid;
