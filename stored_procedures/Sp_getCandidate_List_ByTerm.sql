IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[Sp_getCandidate_List_ByTerm]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    CREATE procedure [dbo].[Sp_getCandidate_List_ByTerm] --''COURSES NO MORE OFFERING-DIPLOMA IN MARKETING''    
(              
...
SELECT * From (SELECT               
ROW_NUMBER() OVER                                 
   (PARTITION BY  CPD.Candidate_Id   order by CPD.Candidate_Id)as num,CPD.Candidate_Id,    
      SG.BloodGroup,  
...
left join  Tbl_Employee EC on (EC.Employee_Id=CPD.CounselorEmployee_id and EC.Employee_Status=0)where Candidate_DelStatus=0)Base   
