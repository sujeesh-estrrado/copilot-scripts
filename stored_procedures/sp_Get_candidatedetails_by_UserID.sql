IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[sp_Get_candidatedetails_by_UserID]') 
    AND type = N'P'
)
BEGIN
    EXEC('
    
CREATE  procedure [dbo].[sp_Get_candidatedetails_by_UserID] --2         sp_Get_candidatedetails_by_UserID 
...
select MD.Candidate_Fname from 
Tbl_Modular_Candidate_Details  MD where 
(@User_Id = MD.Modular_Candidate_Id OR @User_Id = MD.Candidate_Id)  
END
if(@Flag=102)
BEGIN


select Candidate_Fname from 
Tbl_Modular_Candidate_Details where Candidate_Id=@User_Id
