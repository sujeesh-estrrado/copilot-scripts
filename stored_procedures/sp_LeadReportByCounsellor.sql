IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID(N'[dbo].[sp_LeadReportByCounsellor]') 
    AND type = N'P'
)
BEGIN
    EXEC('
   CREATE procedure [dbo].[sp_LeadReportByCounsellor]
(
    @counsellorEmployeeID bigint
)
as
begin
...
WHERE EC.Employee_Id=@counsellorEmployeeID
